...
async function computeInsights(userId: string): Promise<Insight[]> {
  ...
  const [currentExpenses, lastExpenses, goals, latestNetWorth] =
    await Promise.all([
  ...
  const totalCurrent = currentExpenses.reduce(
    (s: any, e: any) => s + e.amount,
    0,
  );
  const totalLast = lastExpenses.reduce((s: any, e: any) => s + e.amount, 0);
  ...
  if (totalLast > 0) {
    ...
    if (change > 0) {
      insights.push({
        icon: "📊",
        text: `You spent ${change}% more this month compared to last month (${formatCurrency(totalCurrent)} vs ${formatCurrency(totalLast)}).`,
        type: "negative",
      });
    } else if (change < 0) {
      insights.push({
        icon: "📊",
        text: `Great! You spent ${Math.abs(change)}% less this month compared to last month.`,
        type: "positive",
      });
    }
  }
  ...
  Object.entries(currentCats).forEach(([cat, amount]) => {
    ...
    if (lastAmount > 0) {
      ...
      if (change > 20) {
        insights.push({
          icon: "🔥",
          text: `You spent ${change}% more on ${label} this month.`,
          type: "negative",
        });
      } else if (change < -20) {
        insights.push({
          icon: "✅",
          text: `${label} expenses decreased by ${Math.abs(change)}%.`,
          type: "positive",
        });
      }
    }
  ...
  if (daysInMonth > 0 && totalCurrent > 0) {
    ...
    insights.push({
      icon: "💡",
      text: `Your average daily spending is ${formatCurrency(daily)}.`,
      type: "neutral",
    });
  }
  ...
  currentExpenses.forEach((e: any) => {
    pmBreakdown[e.paymentMode] = (pmBreakdown[e.paymentMode] || 0) + e.amount;
  });
  ...
  if (topPM && totalCurrent > 0) {
    ...
    insights.push({
      icon: "💳",
      text: `You used ${PAYMENT_MODE_CONFIG[topPM[0]]?.label || topPM[0]} for ${pct}% of your spending.`,
      type: "neutral",
    });
  }
  ...
  goals.forEach((g: any) => {
    ...
    insights.push({
      icon: "🎯",
      text: `${g.icon} ${g.name} progress is at ${progress}%.`,
      type: progress >= 50 ? "positive" : "neutral",
    });
  });
  ...
  if (latestNetWorth) {
    insights.push({
      icon: "💰",
      text: `Your current net worth is ${formatCurrency(latestNetWorth.netWorth)}.`,
      type: "neutral",
    });
  }
...
export default async function InsightsPage() {
  ...
  return <InsightsClient insights={insights} userName={userName} />;
}
