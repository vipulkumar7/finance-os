...
import {
  TrendingUp, TrendingDown, PiggyBank, CreditCard, Wallet, Percent, ArrowUpRight, BarChart3, LineChart as LucideLineChart, Coins, Shield
} from "lucide-react";
    ...
    <div className="glass-card-elevated px-3 py-2 !rounded-lg text-xs">
      ...
      {payload.map((p, i: number) => (
        <div key={i} className="flex items-center gap-2 mt-0.5">
          <div className="w-2 h-2 rounded-full" style={{ background: p.color || "#10b981" }} />
          <span className="text-[var(--text-secondary)]">{p.name}:</span>
          <span className="text-white font-semibold ml-auto">{formatCurrency(p.value)}</span>
        </div>
      ))}
    </div>
  );
...
export default function AnalyticsClient({ expenses, investments, netWorthSnapshots, vehicleExpenses, year, availableYears }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "expenses" | "investments">("overview");
  ...
  const categoryData = useMemo(() => {
    ...
    return Object.entries(cats)
      .map(([k, v]) => ({ name: CATEGORY_CONFIG[k]?.label || k, value: v, color: CATEGORY_CONFIG[k]?.color || "#6b7280" }))
  ...
  const paymentData = useMemo(() => {
    ...
    return Object.entries(pms)
      .map(([k, v]) => ({ name: PAYMENT_MODE_CONFIG[k]?.label || k, value: v, color: PAYMENT_MODE_CONFIG[k]?.color || "#6b7280" }))
      .sort((a, b) => b.value - a.value);
  }, [expenses]);
  ...
  const vehicleData = useMemo(() => {
    ...
    return Object.entries(types)
      .map(([k, v]) => ({ name: VEHICLE_TYPE_CONFIG[k]?.label || k, value: v, color: VEHICLE_TYPE_CONFIG[k]?.color || "#6b7280" }))
      .sort((a, b) => b.value - a.value);
  }, [vehicleExpenses]);
  ...
  const metrics = useMemo(() => {
    ...
    const totalInvested = monthlyInvestments.reduce((sum, inv) => sum + inv.amount, 0);
    const monthsWithExpenses = monthlyExpenses.filter(m => m.amount > 0).length || 1;
    ...
    const avgSavingsRate = validRates.length > 0
      ? Math.round(validRates.reduce((sum, r) => sum + r.rate, 0) / validRates.length)
      : 0;

    // Net Worth Growth this year
    const nwThisYear = netWorthSnapshots.filter(s => s.year === year);
    const nwGrowth = nwThisYear.length > 1
  ...
  }, [expenses, monthlyInvestments, monthlyExpenses, savingsRate, netWorthSnapshots, year, categoryData, paymentData]);

  const hasData = expenses.length > 0 || investments.length > 0 || netWorthSnapshots.length > 0;
    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)]">{year} Financial Report</p>
        </div>
      ...
      {hasData ? (
            ...
            >
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="glass-card-static p-5 metric-accent-green">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Avg Savings Rate</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">{metrics.avgSavingsRate}%</h3>
                      <p className="text-[10px] text-emerald-400 font-medium mt-1">Saved from total inflow</p>
                    </div>
                    <div className="glass-card-static p-5 metric-accent-amber">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Net Worth Growth</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">
                        {metrics.nwGrowth >= 0 ? "+" : ""}{formatCompactCurrency(metrics.nwGrowth)}
                      </h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-1">Snapshot value change</p>
                    </div>
                    <div className="glass-card-static p-5 metric-accent-purple">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Total Invested</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">{formatCompactCurrency(metrics.totalInvested)}</h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-1">Total contributions this year</p>
                    </div>
                  </div>
                        ...
                        <div className="h-[250px]">
                          <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={nwTrend}>
                              <defs>
                                <linearGradient id="nwGradOverview" x1="0" y1="0" x2="0" y2="1">
                              ...
                              <Area type="monotone" dataKey="netWorth" name="Net Worth" stroke="#f59e0b" strokeWidth={2.5} fill="url(#nwGradOverview)" dot={false} />
                            </AreaChart>
                          </ResponsiveContainer>
                        </div>
                      </div>
                          ...
                          <LineChart data={savingsRate}>
                            ...
                            <Tooltip content={({ active, payload, label }) => active && payload?.[0] ? (
                              <div className="glass-card-elevated px-3 py-2 !rounded-lg text-xs">
                                ...
                                <p className="text-white font-semibold">Savings Rate: {payload[0].value}%</p>
                              </div>
                            ) : null} />
                            <Line type="monotone" dataKey="rate" name="Savings Rate" stroke="#10b981" strokeWidth={2.5} dot={{ r: 4, fill: "#10b981", stroke: "#0a0a0f", strokeWidth: 1.5 }} />
                          </LineChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
              ...
              {activeTab === "expenses" && (
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="glass-card-static p-5 metric-accent-green">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Total Spent</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">{formatCurrency(metrics.totalSpent)}</h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-1">Incurred expenses this year</p>
                    </div>
                    <div className="glass-card-static p-5">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Monthly Average</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">{formatCurrency(metrics.avgMonthlySpent)}</h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-1">Across active months</p>
                    </div>
                    <div className="glass-card-static p-5 metric-accent-purple">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Top Category</p>
                      <h3 className="text-xl font-bold text-white mt-1.5 truncate">
                        {metrics.topCategory ? metrics.topCategory.name : "—"}
                      </h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-0.5">
                        {metrics.topCategory ? `${formatCurrency(metrics.topCategory.value)} spent` : ""}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div className="h-[250px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={monthlyExpenses}>
                            <defs>
                              <linearGradient id="expGradOverview" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <Tooltip content={<ChartTooltip />} />
                            <Area type="monotone" dataKey="amount" name="Expenses" stroke="#10b981" strokeWidth={2.5} fill="url(#expGradOverview)" dot={false} />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                    ...
                    {categoryData.length > 0 && (
                      <div className="glass-card-static p-5 flex flex-col justify-between">
                        <div>
                          <h3 className="text-sm font-semibold text-white mb-2">Category Allocation</h3>
                      ...
                      <div className="lg:col-span-3 glass-card-static p-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                          <div className="md:col-span-1 h-[180px]">
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie data={paymentData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={2} dataKey="value" stroke="none">
                          ...
                          <div className="md:col-span-2 grid grid-cols-2 gap-4">
                            {paymentData.map((p) => (
                              <div key={p.name} className="flex flex-col p-2.5 bg-zinc-950/40 border border-zinc-900 rounded-xl">
                                <div className="flex items-center gap-2">
                                  <div className="w-2.5 h-2.5 rounded-full" style={{ background: p.color }} />
                                  <span className="text-[11px] text-[var(--text-muted)] font-semibold truncate">{p.name}</span>
                                </div>
                                <span className="text-sm font-bold text-white mt-1">{formatCurrency(p.value)}</span>
                              </div>
                            ))}
                          </div>
                        </div>
              ...
              {activeTab === "investments" && (
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="glass-card-static p-5 metric-accent-purple">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Total Invested</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">{formatCurrency(metrics.totalInvested)}</h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-1">Growth contributions</p>
                    </div>
                    <div className="glass-card-static p-5">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Monthly Contribution</p>
                      <h3 className="text-2xl font-extrabold text-white mt-1">
                        {formatCurrency(metrics.totalInvested / 12)}
                      </h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-1">Average contribution monthly</p>
                    </div>
                    <div className="glass-card-static p-5 metric-accent-amber">
                      <p className="text-[10px] text-[var(--text-muted)] font-semibold uppercase tracking-wider">Vehicle Spending</p>
                      <h3 className="text-2xl font-bold text-white mt-1.5 truncate">
                        {formatCurrency(vehicleExpenses.reduce((sum, e) => sum + e.amount, 0))}
                      </h3>
                      <p className="text-[10px] text-[var(--text-muted)] mt-0.5">Asset maintenance & fuel</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {vehicleData.length > 0 ? (
                      <div className="glass-card-static p-5">
                        <h3 className="text-sm font-semibold text-white mb-4">Vehicle Expenses Breakdown</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                          <div className="h-[180px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie data={vehicleData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={2} dataKey="value" stroke="none">
                          ...
                          <div className="space-y-1.5">
                            {vehicleData.map((v) => (
                              <div key={v.name} className="flex items-center justify-between text-xs">
                                ...
                                <span className="text-white font-semibold">{formatCurrency(v.value)}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      <div className="glass-card-static p-5 flex items-center justify-center text-center">
                        <p className="text-xs text-[var(--text-muted)]">No vehicle expenses logged for {year}</p>
                      </div>
                    )}
                  </div>
                </>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
          ...
          <div>
            <p className="text-sm font-semibold text-white">No analytics data yet</p>
            <p className="text-xs text-[var(--text-muted)] mt-1">Start adding expenses, investments, or snapshots to see reports.</p>
          </div>
        </div>
      )}
    </div>
